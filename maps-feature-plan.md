...
- **Лупички ➕/➖ във viewer-а** — видими магнифайър-тоглове (долу-център, едри touch таргети): на таблета wheel няма, а pinch не е очевиден за всички. Тап по ➕/➖ въоръжава режим (`.active`), после тап по картата зумва ТОЧНО на това място (клампът [1,8] идва от `zoomAt`); повторен тап по бутона изключва режима. Затварянето и tap-детекцията минават през pointer събития (не `click` — `setPointerCapture` пренасочва производния click към overlay-а). На десктоп native image drag е потиснат (`draggable=false`), за да панва мишката.
